...
class RepoStatsContainer extends Component <RepoStatsProps, RepoStatsState> {
    ...
    sortRepositorys(repos: ReposWithPullRequests[]) {
        return repos.sort((x,y) => { return y.pullRequests.length - x.pullRequests.length })
    }
    ...
    renderRepos(repos: ReposWithPullRequests[]) {
        return (
            <h2>{repos.map((x, index) => {
                return this.renderRepo(x) + (index === this.numberDisplayed-1 ? "" : ", ")
            }).slice(0, this.numberDisplayed)}</h2>
        )
    }
